...
app.post("/index/signin", async (req, res) => {
  ...
  if (!username || !passwd) {
    ...
    try {
      let db = await getDBConnection();
      let query =
        "INSERT INTO Users(username, is_admin, passwd) VALUES(?, false, ?);";
  ...
  if (!itemId || !userId || !inputVal) {
    ...
    try {
      ...
      if (itemType === "Supplies") {
        ...
        if (result1 === 1) {
          // 1 if update was successful, 0 or error statement else
      ...
      if (result2 !== 1) {
        res
          ...
          .type("text")
          .send(
            "Could not add checkout information to the history. Try again later."
          );
      }
    } catch (err) {
      res
...
async function addEquipmentHistory(userId, itemId, inputVal) {
  try {
    let db = await getDBConnection();
    let query =
      "INSERT INTO Checkout(user_id, e_id, checkout_duration)" +
      " VALUES(?, ?, ?);";
...
async function addSupplyHistory(userId, itemId, inputVal) {
  try {
    let db = await getDBConnection();
    let query =
      "INSERT INTO Checkout(user_id, s_id, checkout_amount)" +
      " VALUES(?, ?, ?);";
    let result = await db.run(query, [userId, itemId, inputVal]);
